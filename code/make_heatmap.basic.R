...
prepare_data <- function(obj,outDir="InSilicoCD45sorting",geneExXCells=1,CD45posMinReads=1,genes=NULL,normalize_to_cpm=FALSE){
    ...
    if(isTRUE(normalize_to_cpm)){
        cpm <- calc_cpm(RNA_data)/100
    }
    ...
    RNA_data  <- RNA_data[rownames(RNA_data) %in% genes,]; print(dim(RNA_data))
    ...
    if(isTRUE(normalize_to_cpm)){
        print("Use data normalized to CPM")
        RNA_data         = cpm[rownames(RNA_data),colnames(RNA_data)]
    }
...
make_heatmap <- function(obj,outDir="outdir",geneExXCells=1,CD45posMinReads=1,genes=NULL,show_row_names=F,normalize_to_cpm=FALSE){

    print(outDir)
    
    dir.create(outDir,recursive = T)

    out = prepare_data(obj=obj,outDir=outDir,geneExXCells = geneExXCells,CD45posMinReads = CD45posMinReads,genes=genes,normalize_to_cpm=normalize_to_cpm)
    ...
    make_heatmap_annotations <- function(cell_info,RNA_data){
        ...
        col_ha = HeatmapAnnotation(
                                  'Seurat Cluster'=cell_info[match(colnames(RNA_data),cell_info$cell),]$seurat_clusters,
    ...
    if(isTRUE(normalize_to_cpm)){
        ...
        # use raw RNA data
        p <- Heatmap(log2(RNA_data+1),name="Log CPM",column_title=paste0(ncol(RNA_data)," cells"),top_annotation = col_ha,cluster_rows = T, cluster_columns = F,show_row_dend=F,show_row_names = show_row_names ,show_column_names = F,row_names_side='left',use_raster = TRUE,raster_device="CairoPNG",gap = unit(0.5, "mm"),row_title_gp = gpar(fontsize=10))
        
        pdf(paste0(outDir,"/","heatmap_RNA_logCPM.pdf"),15,10); draw(p); dev.off()

        # use raw RNA data, clustering cells
        p <- Heatmap(log2(RNA_data+1),name="Log CPM",column_title=paste0(ncol(RNA_data)," cells"),top_annotation = col_ha,cluster_rows = T, cluster_columns = T,column_split = cell_info[match(colnames(RNA_data),cell_info$cell),]$seurat_clusters,show_row_dend=F,show_column_dend=F,show_row_names = show_row_names ,show_column_names = F,row_names_side='left',use_raster = TRUE,raster_device="CairoPNG",gap = unit(0.5, "mm"),row_title_gp = gpar(fontsize=10))
        
        pdf(paste0(outDir,"/","heatmap_RNA_logCPM.clustered_cells.pdf"),15,10); draw(p); dev.off()

        # use z-score
        scaled_RNA_data <- t(scale(t(log2(RNA_data+1))))
        scaled_RNA_data <- scales::squish(scaled_RNA_data,range = c(-0.5,0.5))

        p <- Heatmap(scaled_RNA_data,name="Log CPM (Z-score)",column_title=paste0(ncol(RNA_data)," cells"),top_annotation = col_ha,cluster_rows = T, cluster_columns = F,show_row_dend=F,show_row_names = show_row_names ,show_column_names = F,row_names_side='left',use_raster = TRUE,raster_device="CairoPNG",gap = unit(0.5, "mm"),row_title_gp = gpar(fontsize=10))
        
        pdf(paste0(outDir,"/","heatmap_RNA_logCPM_zscore.pdf"),15,10); draw(p); dev.off()
