...
test=FALSE
if(test){
  setwd("/home/zhuy1/my_projects/metaplastic_scRNAseq/run_seurat/")
  path_raw = "/lila/data/reis-filho/data/zhuy1/my_projects/metaplastic_scRNAseq/run_cellRanger/MBC6/MBC6/outs/raw_feature_bc_matrix.h5"
  data_source = "cellranger_atac"
  outdir = "MBC6_highCov/seurat_cellRangerRawData"
}
...
clustering_analysis(obj,outDir=paste0(outdir,'/initial_clustering'),npc=30,immune_markers=NULL,resolution = 0.5, stop_after_marker_genes = FALSE,regress_out_cellcycle=FALSE )

clustering_analysis(obj,outDir=paste0(outdir,'/initial_clustering_regress_out_cell_cycle'),npc=30,immune_markers=NULL,resolution = 0.5, stop_after_marker_genes = FALSE,regress_out_cellcycle=TRUE )


#########################################################################################################
# make FeaturePlot to examine clusters

obj <- readRDS(paste0(outdir,'/initial_clustering/seurat_rna.rds'))

pdf(paste0(outdir,'/initial_clustering/FeaturePlot.pdf'))
FeaturePlot(obj,features='nCount_RNA')
FeaturePlot(obj,features='nFeature_RNA')
FeaturePlot(obj,features='percent.mt')
FeaturePlot(obj,features='MALAT1')
FeaturePlot(obj,features='percent.ribo')
FeaturePlot(obj,features='percent.hb')
FeaturePlot(obj,features='CD45')
FeaturePlot(obj,features='PanCK')
FeaturePlot(obj,features='hybrid_score')
FeaturePlot(obj,features='cxds_score')
FeaturePlot(obj,features='bcds_score')
FeaturePlot(obj,features='scDblFinder.score')
FeaturePlot(obj,features='S.Score')
FeaturePlot(obj,features='G2M.Score')
dev.off()
